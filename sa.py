...
file_name2=['B07322743778B5868475DBE66EEDAC4F']
...
for file in file_name:
	...
	for entry in pe.DIRECTORY_ENTRY_IMPORT:
	    print('Llamadas DLL:')
	    print (entry.dll)
	    ddls.append(entry.dll)
	    print('Llamadas a funciones:')
	    for function in entry.imports:
	    	print ('\t', function.name)
	    	fun_calls.append(function.name)


	# Open a file
	print("TimeDateStamp : " + pe.FILE_HEADER.dump_dict()['TimeDateStamp']['Value'].split('[')[1][:-1])
	f = open("dataset.txt","a")
	f.write(str(file)+","+str(section.Name)+","+str(hex(section.VirtualAddress))+","+str(hex(section.Misc_VirtualSize))+","+str(section.SizeOfRawData)+","+str(ddls)+","+str(fun_calls)+"\n")
	f.close()
